    ...
    let itemquery = "query { items (ids: [" + req.body.event.pulseId + "]) { name }}"; 
      ...
      fetch ("https://api.monday.com/v2", {
        ...
        headers: {
         ...
         .then(res => res.json())
         .then(res => {
          let bodyText = "<p>### AUTO COPIED FROM SUB ITEM ###</p><p><span style='color: rgb(0, 163, 89);' data-redactor-style-cache='color: rgb(0, 163, 89);'>" + uname + "님이 서브아이템 '"+ res.data.items[0].name +"'에 업데이트한 내용입니다.</span></p>" + req.body.event.body.replace("\\", '').replace(/\"/g, "'");
